...
Keep learning! 📚✨`;
  ...
  async sendCertificate(to: string, name: string, trackName: string, completionDate: string) {
    const message = \`🏆 *CONGRATULATIONS!*

*${name}*, you've successfully completed:
...
🔗 Download: https://skillboost.co.ke/certificate/${name.replace(' ', '-')}-${trackName.replace(' ', '-')}.pdf

Share your achievement:
📱 Post on social media with #SkillBoostGraduate
💼 Add to your LinkedIn profile
📄 Include in your CV/Resume

Ready for your next challenge? Reply "TRACKS\" to see what's available!
...
Congratulations again! 🎉`;
  ...
  async sendProgressUpdate(to: string, name: string, weeklyStats: any) {
    const message = \`📊 *Weekly Progress Report*

Hi ${name}! Here's your learning summary:

*This Week:*
✅ Lessons Completed: ${weeklyStats.lessonsCompleted}
⏱️ Learning Time: ${weeklyStats.totalMinutes} minutes
🎯 Streak: ${weeklyStats.currentStreak} days
📈 Overall Progress: ${weeklyStats.overallProgress}%

*Achievements:*
${weeklyStats.badges.map((badge: string) => `🏅 ${badge}`).join('\n')}

*Next Week's Goal:*
Complete ${weeklyStats.nextWeekGoal} lessons to maintain your streak!

Keep up the great work! 💪

---
Reply "DETAILED\" for full progress report
Reply "PAUSE\" if you need a break`;
    ...
    try {
      ...
      if (value?.messages) {
        ...
        switch (messageBody) {
          ...
          default:
            // Handle quiz answers or general responses
            return {
  ...
  async sendHelpMenu(to: string) {
    const message = \`📞 *SkillBoost Kenya - Help Center*

*Available Commands:*
📚 "PROGRESS" - View your learning stats
⏸️ "PAUSE" - Pause daily lessons
▶️ "RESUME" - Resume daily lessons  
➡️ "NEXT" - Preview tomorrow's lesson
🎯 "TRACKS" - View available courses
💰 "PAID" - Confirm payment made

*Having Issues?*
📱 Call: +254 700 123 456
📧 Email: help@skillboost.co.ke
💬 WhatsApp: Just reply to this message
